...
def draw_labels(frame, boxes, confidences, class_ids, classes):
    ...
    for i in range(len(boxes)):
        if i in indexes:
            ...
            if label == "person":  # Count only persons
                person_count += 1
            color = (0, 255, 0)  # Green bounding box
            cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
            cv2.putText(frame, label, (x, y + 30), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)

    cv2.putText(frame, f'Persons Count: {person_count}', (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    return person_count
...
def main():
    ...
    while True:
        ...
        person_count = draw_labels(frame, boxes, confidences, class_ids, classes)
