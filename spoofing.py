...
from keras.layers import AveragePooling2D, MaxPooling2D # type: ignore
...
image = cv2.imread('images/images.jpg')
print(f"[LOG] Loaded image shape: {image.shape} with dtype: {image.dtype}")

process_and_encode(images)
# image = cv2.imread('faces')
...
#     video_capture = VideoStream(src=0).start()  # src=0 for the default webcam
#     eyes_detected = defaultdict(str)  # To keep track of eye status for each detected face
...
#                                    left_eye_detector, right_eye_detector, encodings, eyes_detected)
        
#         # Display the output frame
#         cv2.imshow("Real-Time Spoofing Detection", frame)
...
# # Start the real-time spoofing detection
# start_video_stream()
...
#     known_encodings = []
#     known_names = []
#     encodings = {}  # Initialize encodings dictionary here
...
#         # Detect face in the image and get its location (square box coordinates)
#         boxes = face_recognition.face_locations(image, model='cnn')

#         # Encode the face into a 128-d embedding vector
#         encoding = face_recognition.face_encodings(image, boxes)

#         # The person's name is the name of the folder where the image comes from
#         name = image_path.split(os.path.sep)[-2]

#         # Check if any face encodings were found before adding
#         if len(encoding) > 0:
#             known_encodings.append(encoding[0])
#             known_names.append(name)

#     # Save encodings only if there are any known encodings
#     if known_encodings:
#         encodings = {"encodings": known_encodings, "names": known_names}
#         np.save('encodings.npy', encodings)
#     else:
#         print("[ERROR] No faces were encoded. Check the dataset and try again.")
...
# # def real_time_spoofing_detection(model, face_detector, open_eyes_detector, left_eye_detector, right_eye_detector, data):
# #     # Initialize webcam video stream
# #     video_capture = VideoStream(src=0).start()
# #     eyes_detected = defaultdict(str)
    
# #     print("[LOG] Starting video stream for real-time spoofing detection...")
...
# #         # Process the frame and detect spoofing
# #         processed_frame = detect_and_display(
# #             model, frame, face_detector, open_eyes_detector, left_eye_detector,
# #             right_eye_detector, data, eyes_detected
# #         )
        
# #         # Display the resulting frame
# #         cv2.imshow("Real-Time Spoofing Detection", processed_frame)
...
# # # Assuming you have already initialized the model and detectors in `init()` and encoded known faces.
# # (model, face_detector, open_eyes_detector, left_eye_detector, right_eye_detector, images) = init()
# # data = process_and_encode(images)

# # # Start real-time spoofing detection
# # real_time_spoofing_detection(
# #     model, face_detector, open_eyes_detector, left_eye_detector, right_eye_detector, data
# # )
# import cv2

# def real_time_spoofing_detection(model, face_detector, open_eyes_detector, left_eye_detector, right_eye_detector, data):
#     # Initialize video capture from the webcam (use `0` for the default camera)
#     video_capture = cv2.VideoCapture(0)  # 0 is for the primary camera; replace with video file path if needed

#     # Dictionary to store eye status history for each detected person
#     eyes_detected = defaultdict(str)

#     print("[LOG] Starting real-time spoofing detection...")

#     while True:
#         # Capture frame-by-frame
#         ret, frame = video_capture.read()
#         if not ret:
#             print("[ERROR] Failed to grab frame from video capture")
#             break

#         # Process and display the frame
#         processed_frame = detect_and_display(
#             model, frame, face_detector, open_eyes_detector,
